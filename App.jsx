        ...
        {step===2&&(
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
            ...
            <div style={{ padding:"13px 16px",background:"rgba(255,255,255,0.04)",borderRadius:14,border:"1px solid rgba(255,255,255,0.07)" }}>
              <div style={{ fontSize:12,color:"rgba(255,255,255,0.38)",lineHeight:1.6,fontFamily:DM }}>By continuing you agree to our <span onClick={()=>window.open('https://app.termly.io/policy-viewer/policy.html?policyUUID=8d3c678c-644d-473e-b115-f80a96727387','_blank')} style={{ color:C.accent,cursor:"pointer",textDecoration:"underline" }}>Terms of Use</span> and <span onClick={()=>window.open('https://app.termly.io/policy-viewer/policy.html?policyUUID=fe689e0e-3f09-47ba-b48d-372df55d04a7','_blank')} style={{ color:C.accent,cursor:"pointer",textDecoration:"underline" }}>Privacy Notice</span>.</div>
            </div>
          </div>
        )}
        <div style={{ height:24 }}/>
...
function PrivacyScreen({ onAccept }) {
  ...
  const linkBtn = { display:"block", width:"100%", padding:"14px 16px", borderRadius:12, border:"1px solid rgba(75,193,160,0.3)", background:"rgba(75,193,160,0.08)", color:C.accent, fontFamily:DM, fontSize:15, cursor:"pointer", textAlign:"left", marginBottom:12, boxSizing:"border-box" };
  return (
    <div style={{ flex:1, display:"flex", flexDirection:"column", background:"#021a16", position:"relative", overflow:"hidden" }}>
      <BlobBackground overlayStrength="0.88" />
      <div style={{ flex:1, display:"flex", flexDirection:"column", justifyContent:"center", padding:"48px 28px 24px", position:"relative" }}>
        <div style={{ fontFamily:DM, fontSize:13, color:C.accent, fontWeight:700, letterSpacing:1, textTransform:"uppercase", marginBottom:12 }}>Before you continue</div>
        <div style={{ fontFamily:DM, fontSize:26, fontWeight:700, color:"#fff", letterSpacing:-0.5, marginBottom:16 }}>Privacy & Terms</div>
        <div style={{ fontFamily:DM, fontSize:15, color:"rgba(255,255,255,0.55)", lineHeight:1.75, marginBottom:32 }}>
          By using Chins you agree to our Privacy Notice and Terms of Use. Please read them before continuing.
        </div>
        <button onClick={()=>window.open('https://app.termly.io/policy-viewer/policy.html?policyUUID=fe689e0e-3f09-47ba-b48d-372df55d04a7','_blank')} style={linkBtn}>
          🔒 Privacy Notice →
        </button>
        <button onClick={()=>window.open('https://app.termly.io/policy-viewer/policy.html?policyUUID=8d3c678c-644d-473e-b115-f80a96727387','_blank')} style={{ ...linkBtn, marginBottom:32 }}>
          📄 Terms of Use →
        </button>
        <label style={{ display:"flex", alignItems:"flex-start", gap:12, cursor:"pointer", marginBottom:20 }}>
          <div onClick={()=>setChecked(c=>!c)} style={{ width:22, height:22, borderRadius:6, border:"2px solid "+(checked?C.accent:"rgba(255,255,255,0.25)"), background:checked?C.accent:"transparent", flexShrink:0, display:"flex", alignItems:"center", justifyContent:"center", marginTop:1, cursor:"pointer" }}>
...
function SafetyScreen({ onContinue }) {
  ...
  const tips = [
    ...
    },
    {
      ...
      body:"Always meet someone new in a busy public place. Let someone you trust know where you're going, who you're meeting, and when to expect you back.",
    },
    {
      icon:"🚨",
  ...
  return (
    <div style={{ flex:1, display:"flex", flexDirection:"column", background:"#021a16", position:"relative", overflow:"hidden" }}>
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"52px 24px 16px", position:"relative" }}>
        ...
        <div style={{ marginBottom:profile.values?.length > 0 ? 16 : 28 }}>
        ...
        {profile.values?.length > 0 && (
          <div style={{ marginBottom:28 }}>
            <div style={{ fontFamily:DM, fontSize:12, color:"rgba(255,255,255,0.35)", fontWeight:600, letterSpacing:1, textTransform:"uppercase", marginBottom:10 }}>What matters to you</div>
            <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
              {(Array.isArray(profile.values) ? profile.values : [profile.values]).map((v,i)=>(
                <div key={i} style={{ padding:"7px 14px", borderRadius:20, background:"rgba(75,193,160,0.08)", border:"1px solid rgba(75,193,160,0.2)", fontFamily:DM, fontSize:13, color:C.accent }}>{v}</div>
              ))}
            </div>
          </div>
        )}
      ...
      try{
        ...
        if(authToken && userId) {
          sb.upsertProfile(authToken, userId, {
            ...
            values: Array.isArray(parsed.values) ? parsed.values : (parsed.values ? [parsed.values] : []),
            values_openness: parsed.valuesOpenness || "",
