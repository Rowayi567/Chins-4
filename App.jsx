      ...
      {!isPrivate && connectionCount === 0 && userProfile && (
        <div style={{ margin:"12px 16px 0", padding:"14px 16px", background:"rgba(75,193,160,0.12)", borderRadius:16, border:"1px solid rgba(75,193,160,0.25)", flexShrink:0 }}>
          <div style={{ display:"flex", gap:10, alignItems:"flex-start" }}>
            <ReedCharacter mood="idle" size={22}/>
            <div>
  ...
  if(activeChat) return (
    <div style={{ flex:1, display:"flex", flexDirection:"column", background:"#021a16", overflow:"hidden" }}>
      <div style={{ padding:"14px 20px 12px", borderBottom:"1px solid rgba(255,255,255,0.08)", display:"flex", alignItems:"center", gap:12, flexShrink:0 }}>
        ...
        <div style={{ marginLeft:"auto", padding:"4px 10px", borderRadius:20, background:"rgba(75,193,160,0.1)", border:"1px solid rgba(75,193,160,0.2)" }}>
  ...
  return (
    <div style={{ flex:1, display:"flex", flexDirection:"column", background:C.bg, overflow:"hidden" }}>
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"12px 16px 24px" }}>
        {starters.map((s,i) => (
          <div key={s.id} onClick={()=>!s.isOwn&&openChat(s)} style={{ background:s.isOwn?"rgba(75,193,160,0.08)":"rgba(255,255,255,0.05)", borderRadius:18, border:"1.5px solid "+(s.isOwn?"rgba(75,193,160,0.3)":"rgba(255,255,255,0.08)"), padding:"16px", marginBottom:10, cursor:s.isOwn?"default":"pointer", transition:"background 0.2s" }}>
            ...
            <div style={{ fontFamily:DM, fontSize:15, color:"#fff", lineHeight:1.6, marginBottom:12 }}>{s.text}</div>
        ...
        {starters.length === 0 && (
          <div style={{ textAlign:"center", padding:"60px 32px" }}>
            <div style={{ fontSize:48, marginBottom:16 }}>⚡</div>
            <div style={{ fontFamily:DM, fontSize:18, fontWeight:700, color:"#fff", marginBottom:8 }}>No sparks yet</div>
            <div style={{ fontFamily:DM, fontSize:14, color:"rgba(255,255,255,0.4)", lineHeight:1.7 }}>Be the first to start a conversation</div>
          </div>
        )}
      </div>
    </div>
...
function TabIcon({ id, active }) {
  ...
  switch(id) {
    ...
    case "mingle": return (
      <svg width="20" height="20" viewBox="0 0 22 22" fill="none">
        <path d="M13 2L4 13h7l-2 7 9-11h-7l2-9z" stroke={c} strokeWidth={sw} strokeLinecap="round" strokeLinejoin="round" fill="none"/>
      </svg>
    );
    case "profile": return (
      <svg width="20" height="20" viewBox="0 0 22 22" fill="none">
...
export default function ChinsApp() {
  ...
  const tabs = [
    ...
    { id:"mingle",  label:"Sparks",  color:"#4BC1A0" },
          ...
          {screen==="main"&&(
            <>
              <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden", position:"relative" }}>
                ...
                {tab==="mingle"&&<SparkScreen userProfile={profile}/>}
                {tab==="reed"&&(
                  <ReedChat
                    msgs={msgs} loading={loading} input={input} setInput={setInput}
